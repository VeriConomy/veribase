...
- [Development guidelines](#development-guidelines)
    - [General Veribase](#general-veribase)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Veribase uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play veribase" on the test network, if you
...
Veribase is a multi-threaded application, and deadlocks or other
...
To profile a running vericoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep vericoind` -- sleep 60
```
...
Veribase can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Veribase, where everyone uses
...
pay attention to for reviewers of Veribase code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Veribase uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Veribase, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Veribase so that they can be integrated
...
file descriptors, it will cause problems with Veribase's `select()` loop, because
...
compatibility between nodes. This happened in Vericoin 1.7.2 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Veribase's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `vericoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `vericoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Veribase, the wallet was always
