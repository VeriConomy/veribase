# PSBT Howto for Veribase

Since Veribase 2.0.0, an RPC interface exists for Partially Signed Bitcoin
...
PSBT is an interchange format for Vericoin and Verium transactions that are not fully signed
...
Overall, the construction of a fully signed Vericoin / Verium transaction goes through the
following steps:
...
- An **Extractor** produces a valid Vericoin / Verium transaction (in network format)
...
#### Multisig with multiple Veribase instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Veribase. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `vericoin-cli` / `verium-cli` is used.
...
Later, when *V* VRC / VRM has been received on *Amulti*, and Bob and Carol want to
