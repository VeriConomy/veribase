...
The data directory is the default location where the Vericoin / Verium files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.vericonomy/`
macOS    | `$HOME/Library/Application Support/Vericonomy/`
Windows  | `%APPDATA%\Vericonomy\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `vericonomy.conf` file, is chain-specific. This means the actual data directory paths for vericoin and verium cases differ:

Chain option        | Data directory path
--------------------|--------------------
`vericoin`          | *path_to_datadir*`/vericoin/`
`verium`          | *path_to_datadir*`/verium/`
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Vericoin / Verium blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `vericonomy.conf`        | Contains [configuration settings](vericonomy-conf.md) for `vericoind` / `vericoin-qt` / `veriumd` / `verium-qt`; can be specified by `-conf` option
`./`               | `vericoin.pid` or `verium.pid`        | Stores the process ID (PID) of `vericoind` / `vericoin-qt` / `veriumd` / `verium-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by the daemon; can be specified by `-debuglogfile` option
...
`vericoin-qt` / `verium-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the Veribase:
