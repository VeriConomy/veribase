...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "Note: Use \"getwalletinfo\" to query the scanning progress.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The " + Params().GetCoinName() + " address (or hex-encoded script)"},
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid " + Params().GetCoinName() + " address or script");
        }
    }
    if (fRescan)
    {
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The " + Params().GetCoinName() + " address for the private key"},
                },
                RPCResult{
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, RPCArg::Optional::NO, "The filename with path (either absolute or relative to " + Params().GetCoinName() + "d)"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    // produce output
    file << strprintf("# Wallet dump created by " + Params().GetCoinName() + " %s\n", CLIENT_BUILD);
